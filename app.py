...
def register():
    ...
    if not username or not password:
        return jsonify({'success': False, 'message': 'Nome de utilizador e palavra-passe são obrigatórios.'}), 400
    ...
    try:
        ...
        return jsonify({'success': False, 'message': 'Este nome de utilizador já existe.'}), 409
    finally:
        conn.close()
...
def login():
    ...
    if user and check_password_hash(user['password'], password):
        ...
        return jsonify({'success': True, 'message': 'Login bem-sucedido!', 'username': user['username']}), 200
    else:
...
def add_favorite_game():
    ...
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Login necessário para favoritar jogos.'}), 401
